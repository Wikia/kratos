...
Similarly, Bcrypt wrapped with AES can be also configured. In addition to standard to
Bcrypt `cost` parameter (default value is 12) it requires AES `key` to be specified.
Since we want to use specific AES variant (namely AES-256) the `key` parameter should
be 32-bytes long (see: [docs](https://golang.org/pkg/crypto/aes/#NewCipher)).
...
hashers:
  bcryptaes:
    const: 12
    key: ABCDEFGHIJKLMNOPQRSTUVXYZ1234567
```
