...
[docs](https://golang.org/pkg/crypto/aes/#NewCipher)). `key` parameter is
defined as an array to allow easy key rotation (similarly how secrets are
implemented) where first key is treated as "current" and others are treated as
legacy one and are used only to verify hashes that were created using older
keys.
