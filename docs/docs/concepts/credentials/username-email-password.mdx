...
- [BCrypt-AES-256](https://dropbox.tech/security/how-dropbox-securely-stores-your-passwords)
...
Similarly, Bcrypt wrapped with AES can be also configured. In addition to
standard to Bcrypt `cost` parameter (default value is 12) it requires AES `key`
to be specified. Since we want to use specific AES variant (namely AES-256) the
`key` parameter should contain 32-bytes long strings (see:
[docs](https://golang.org/pkg/crypto/aes/#NewCipher)). `key` parameter is defined
as an array to allow easy key rotation (similarly how secrets are implemented)
where first key is treated as "current" and others are treated as legacy one and
are used only to verify hashes that were created using older keys.
