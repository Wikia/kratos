...
(before identity is being saved) and post-persist phase (after it is saved). You
can specify a single phase for a hook to be run in though. Possible options are
`all` (the default), `pre-persist` and `post-persist`.
